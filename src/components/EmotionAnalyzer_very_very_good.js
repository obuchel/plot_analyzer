...
// EmotionAnalyzer class with caching
class EmotionAnalyzer {
  ...
  analyzeEmotionWithKeywords(inputText) {
    ...
    emotionScores.neutral *= this.neutralWeight;
  ...
  async analyzeEmotion(inputText, options = {}) {
    try {
      ...
      // Create a cache key that includes the text and neutral weight
      const cacheKey = `${inputText}_${this.neutralWeight}`;
      ...
      {result && (
        <div className="mb-6 p-4 bg-gray-100 rounded-lg">
          <h3 className="text-xl font-bold mb-3" style={{ color: EMOTION_COLORS[result.predictedEmotion] }}>
            ...
            {result.usedFallback && <span className="ml-2 text-sm text-gray-500">(using keyword analysis)</span>}
          </h3>
