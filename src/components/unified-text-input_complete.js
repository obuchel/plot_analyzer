        ...
        <div className="analysis-results">
          ...
          <div className="result-section">
            ...
            <div className="character-analysis">
              <h4>Character Danger Assessment</h4>
              <div className="characters-grid">
                {Object.values(analysisResults.characterDanger.characters).map((character, index) => (
                  <div key={index} className={`character-card danger-${character.dangerLevel}`}>
                    <div className="character-header">
                      <h5>{character.name}</h5>
                      <span className={`danger-badge ${character.dangerLevel}`}>
                        {character.dangerLevel} danger
                      </span>
                    </div>
                    <div className="character-stats">
                      <div className="stat">
                        <span className="stat-label">Danger Score:</span>
                        <span className="stat-value">{(character.dangerScore * 100).toFixed(1)}%</span>
                      </div>
                      <div className="stat">
                        <span className="stat-label">Dominant Emotion:</span>
                        <span className="stat-value">{character.dominantEmotion}</span>
                      </div>
                      <div className="stat">
                        <span className="stat-label">Appearances:</span>
                        <span className="stat-value">{character.appearances}</span>
                      </div>
                    </div>
                    {character.dangerMoments.length > 0 && (
                      <div className="danger-moments">
                        <strong>High Tension Moments:</strong>
                        {character.dangerMoments.map((moment, idx) => (
                          <div key={idx} className="danger-moment">
                            <small>Paragraph {moment.paragraphIndex + 1}: {moment.text}</small>
                            <span className="tension-level">({(moment.tension * 100).toFixed(1)}% tension)</span>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {analysisResults.characterDanger.dangerHotspots.length > 0 && (
                <div className="danger-hotspots">
                  <h4>Narrative Danger Hotspots</h4>
                  {analysisResults.characterDanger.dangerHotspots.map((hotspot, index) => (
                    <div key={index} className="hotspot-card">
                      <div className="hotspot-header">
                        <strong>Paragraph {hotspot.paragraphIndex + 1}</strong>
                        <span className="tension-score">{(hotspot.tension * 100).toFixed(1)}% tension</span>
                      </div>
                      <div className="hotspot-text">{hotspot.text}</div>
                      <div className="hotspot-emotion">Emotion: {hotspot.emotion}</div>
                    </div>
                  ))}
                </div>
              )}
