...
class EmotionAnalyzer {
  ...
  // Initialize and load model - with caching
  async initialize() {
    ...
    try {
      ...
      try {
        ...
        this.log(`Session creation error: ${sessionError.message}`);
        
        // If there's an error with the session, try with a simpler configuration
        try {
          this.session = await ort.InferenceSession.create(CACHE.modelBuffer, { executionProviders: ['wasm'] });
          this.log(`Model loaded with simplified options`);
          ...
          this.log(`Simplified loading also failed: ${fallbackError.message}`);
          throw fallbackError;
        }
      }
    } catch (error) {
      ...
      // Fall back to the keyword-based approach if the model can't be loaded
      this.log('Falling back to keyword-based emotion analysis');
  ...
  // Set neutral weight (for backward compatibility)
  setNeutralWeight(weight) {
    this.config.neutralWeight = parseFloat(String(weight));
    this.log(`Set neutral weight to ${this.config.neutralWeight}`);
  }

  // Keyword-based analysis (fallback)
  analyzeEmotionWithKeywords(inputText) {
    ...
    const EMOTION_KEYWORDS = {
      anger: ['anger', 'angry', 'mad', 'furious', 'rage', 'hate', 'hatred', 'outrage', 'annoyed', 'irritated', 'bitter'],
      disgust: ['disgust', 'disgusted', 'gross', 'revolting', 'repulsive', 'nasty', 'yuck', 'distaste'],
      fear: ['fear', 'afraid', 'scared', 'terrified', 'frightened', 'worried', 'anxious', 'panic', 'dread'],
      joy: ['joy', 'happy', 'happiness', 'pleased', 'delighted', 'cheerful', 'glad', 'excited', 'love', 'enjoyed'],
      sadness: ['sad', 'upset', 'unhappy', 'miserable', 'depressed', 'gloomy', 'sorrow', 'grief', 'tears', 'cry'],
      surprise: ['surprise', 'surprised', 'shocking', 'shock', 'astonished', 'amazed', 'unexpected', 'stunned'],
      neutral: ['said', 'went', 'going', 'come', 'coming', 'went', 'see', 'saw', 'look', 'report', 'told', 'says']
    };
    
    // Convert to lowercase for keyword matching
    const lowerText = inputText.toLowerCase();
    
    // For each emotion, look for keywords
    EMOTION_LABELS.forEach(emotion => {
    ...
    // Add base values to ensure some distribution
    EMOTION_LABELS.forEach(emotion => {
      // Add a small base value (0.1) to all emotions
      emotionScores[emotion] += 0.1;
      
      // If no emotions were detected, boost neutral
      if (emotion === 'neutral' && Object.values(emotionScores).reduce((sum, val) => sum + val, 0) < EMOTION_LABELS.length * 0.2) {
        emotionScores[emotion] += 0.5;
      }
    });
    
    // Apply neutral weight
    emotionScores.neutral *= this.config.neutralWeight;
    
    // Convert scores to logits-like values
    const logits = EMOTION_LABELS.map(emotion => {
  ...
  // Main analyze function - with caching and fallback
  async analyzeEmotion(inputText, options = {}) {
    try {
      ...
      // Try model-based analysis first, and fallback to keyword if needed
      let logits;
      
      if (this.useKeywordFallback) {
        // Use keyword-based analysis
        this.log('Using keyword-based analysis (fallback mode)');
        logits = this.analyzeEmotionWithKeywords(inputText);
      } else {
        ...
        try {
          ...
          // If model inference fails, fall back to keyword-based analysis
          this.log(`Model inference error: ${modelError.message}. Falling back to keyword analysis.`);
          this.useKeywordFallback = true;
          logits = this.analyzeEmotionWithKeywords(inputText);
        }
      }
      ...
      if (emotionResults[0].probability < this.config.confidenceThreshold) {
        ...
        if (neutralResult) {
          ...
          confidence = neutralResult.probability;
          ...
          confidence = emotionResults[0].probability;
        }
      } else {
      ...
      const result = {
        ...
        tokenization,
        usedFallback: this.useKeywordFallback,
                 ...
                 <div className="mb-4">
                   ...
                   {result.usedFallback && (
                     <div className="mt-1 text-amber-600 text-sm">
                       Using fallback keyword-based analysis (model not available)
                     </div>
                   )}
                 </div>
